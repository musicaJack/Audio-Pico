...
bool ILI9488HAL::initializeGPIO() {
    ...
    if (config_.pin_bl != 255) {
        ...
        pwm_config_set_clkdiv(&pwm_cfg, 4.0f);  // Divide clock by 4
        pwm_config_set_wrap(&pwm_cfg, 255);     // 8-bit resolution (0-255)
        pwm_init(slice_num, &pwm_cfg, true);    // Initialize and enable
        
        setBacklightBrightness(255);  // Full brightness
        printf("Backlight PWM initialized on GPIO %d (slice %d)\n", config_.pin_bl, slice_num);
    }
